...
TLAC plainly, links to `jeffkazzee.dev`, keeps the primary nav pointed at
`/about/`, and passes desktop/mobile layout checks without rejected copy.

## Evidence

- `curl.exe -I https://jeffkazzee.dev`: returned HTTP 200.
- `npm run check`: passed with 0 errors, 0 warnings, 0 hints.
- `npm run build`: passed and generated `/about/`.
- Static route smoke: passed for one H1, TLAC definition, belongs/broader-work
  sections, personal-site link, projects link, homepage CTA, nav link, and
  rejected-copy scan.
